...
?>
	...
	$strSQL = "
		SELECT 
				...
				t1.verifiedemail,t1.verifiedphone,t1.defaultcid,t1.pid,t1.2fasms,
				t2.access_name,
				t3.companyname
	...
	while ($arrRow = mysqli_fetch_row($arrRS)) {
		...
		$rowUserAdded		= date('Y-m-d H:i', $arrRow[5]);
		$rowUserLastLogin	= date('Y-m-d H:i', $arrRow[6]);
		...
		$row2FASMS = $arrRow[25]; // 2fasms

		$rowAccessName	= !empty($arrRow[26]) ? $arrRow[26] : $gloNULL;

		$rowUserFullname = $rowFName . " " . $rowSName; //Show users firstname and lastname	

		$rowResellerName	= !empty($arrRow[27]) ? $arrRow[27] : $gloNULL;
